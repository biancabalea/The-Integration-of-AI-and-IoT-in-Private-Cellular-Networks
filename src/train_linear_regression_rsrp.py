...
def main():
    print("=== LINEAR REGRESSION: RSRP PREDICTION ===")
    ...
    joblib.dump(model, MODEL_DIR / "linear_regression_rsrp.pkl")
    ...
    predictions_df.to_csv(
        RESULTS_DIR / "predictions" / "linear_regression_rsrp_predictions.csv",
        index=False
    )
    ...
    metrics_df.to_csv(
        RESULTS_DIR / "metrics" / "linear_regression_rsrp_metrics.csv",
        index=False
    )
    ...
    plt.title("Linear Regression - RSRP: real vs prezis")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "figures" / "linear_regression_rsrp_real_vs_pred.png", dpi=300)
    plt.close()
